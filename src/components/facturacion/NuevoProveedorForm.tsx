...
export default function NuevoProveedorForm() {
  return (
    <form
      action={crearProveedorAction}
      className="mt-6 space-y-4 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"
    >
      <Campo name="razonSocial" label="Razon social" placeholder="Ej. Distribuidora ABC S.A.C." />
      <Campo name="ruc" label="RUC (opcional)" required={false} />
      <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
        <input type="checkbox" name="esPersonaNatural" value="1" className="rounded border-slate-300 dark:border-slate-700" />
